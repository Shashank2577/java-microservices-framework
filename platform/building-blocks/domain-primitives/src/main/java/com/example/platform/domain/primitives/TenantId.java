 ...
 *
 * <p>The framework's multi-tenancy posture (schema-per-tenant) treats this as the unit of
 * isolation: every repository query, every Kafka message, every structured log line carries a
 * {@code TenantId}.
 *
...
public record TenantId(UUID value) {
  ...
  /**
   * Construct from a string representation. The string must be a valid lowercase UUID (RFC 4122
   * canonical form).
   *
